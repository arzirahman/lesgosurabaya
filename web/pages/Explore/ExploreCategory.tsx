...
import { lazy, useState, Suspense, JSX, useEffect } from 'react';
import Footer from '../../components/Footer';

const Wisata = lazy(() => import('./Wisata'));
const Hotel = lazy(() => import('./Hotel'));
const Umkm = lazy(() => import('./Umkm'));
const Lifestyle = lazy(() => import('./Lifestyle'));

const categoryMap: Record<
    string,
    { banner: string; Component: React.LazyExoticComponent<() => JSX.Element> }
> = {
    tourism: { banner: wisata, Component: Wisata },
    hospitality: { banner: hospital, Component: Hotel },
    lifestyle: { banner: lifestyle, Component: Lifestyle },
    umkm: { banner: umkm, Component: Umkm },
};
...
export default function ExploreCategory() {
    ...
    const current = category ? categoryMap[category] : undefined;
        ...
        <div className="w-full flex flex-col overflow-hidden">
            <div className="w-full relative flex flex-col h-[558px] overflow-hidden">
                ...
                <div className='pt-[100px] w-full h-full max-w-[1440px] mx-auto z-[1] flex flex-col justify-center items-center text-white'>
                    <span className='font-semibold text-xl mb-6'>LESGOSURABAYA | {category?.toUpperCase()}</span>
                    <span className='font-extrabold text-[128px] mb-[100px] leading-[76px] text-shadow-lg'>{category?.toUpperCase()}</span>
