...
import dummyData from '../../../assets/dummy/tourism.json';
    ...
    return (
        <>
            <div className='flex bg-[#B8C5C65E] rounded-[20px] w-full items-center'>
                ...
                <input value={search} onChange={(e) => { setSearch(e.target.value) }} className='font-bold py-2 pl-[10px] pr-[25px] rounded-r-[20px] text-2xl text-[#575757] w-full' type='text' placeholder={'Explore TOURISM'} />
            </div>
            <div className="w-full grid grid-cols-4 gap-x-[20px] gap-y-[70px] h-[944px]">
                {pageData.map((item, index) => (
                    <ExploreCard key={`wisata.${item.name}.${index}`} {...item} />
                ))}
            </div>
